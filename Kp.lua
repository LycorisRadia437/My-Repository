...
-- Tối ưu hóa thuật toán Teleport tránh bị kẹt hoặc rơi xuống void
local function TeleportToInstance(targetInstance)
    ...
    if isAlive and rootPart and targetInstance then
        ...
        if part then
            -- Thêm thuộc tính bất tử vận tốc tạm thời để tránh bị Anti-cheat phát hiện dịch chuyển tức thời quá nhanh
...
local BasicOptionsSection = UniversalTab:CreateSection("Universal Basic Options")
...
BasicOptionsSection:AddButton({
    ...
    Callback = function()
        if setclipboard then setclipboard("https://discord.gg") end
    end
})

-- Universal -> Weapon Assist
local WeaponAssistSection = UniversalTab:CreateSection("Weapon Assist")
...
WeaponAssistSection:AddDropdown({
    ...
    List = {"Closest", "Torso", "Head", "HumanoidRootPart"},
    Callback = function(option) Settings.AssistTargetPart = option end
})

-- Universal -> Movement
local MovementSection = UniversalTab:CreateSection("Movement Options")
...
MovementSection:AddSlider({
    Name = "Walk Speed",
    Flag = "WalkSpeed",
    Min = 16, Max = 100, Value = 16, Precise = 1, -- Đã tăng giới hạn tối đa phù hợp với bản mới
    Callback = function(value)
        Settings.WalkSpeed = value
        if Settings.UseWalkSpeed then
            local isAlive, _, hum = GetAliveCharacter(LocalPlayer)
            if isAlive and hum then hum.WalkSpeed = value end
        end
    end
})

-- Loop giữ WalkSpeed ổn định không bị game reset liên tục
RunService.PostSimulation:Connect(function()
    if Settings.UseWalkSpeed then
        local isAlive, _, hum = GetAliveCharacter(LocalPlayer)

      if isAlive and hum and hum.WalkSpeed ~= Settings.WalkSpeed thenhum.WalkSpeed = Settings.WalkSpeedendendend)MovementSection:AddToggle({Name = "No Jump Cooldown",Flag = "NoJumpCooldown",Callback = function(enabled) Settings.NoJumpCooldown = enabled end})-- Human -> Basiclocal HumanBasicSection = HumanTab:CreateSection("Human Basic Options")HumanBasicSection:AddToggle({Name = "Bypass Barrier",Flag = "BarrierBypass",Callback = function(enabled)Settings.BarrierBypass = enabledfor _, barrier in ipairs(Workspace:GetDescendants()) doif barrier:IsA("BasePart") and (barrier.Name:lower():find("barrier") or barrier.Name:lower():find("gate")) thenbarrier.CanCollide = not enabledendendend})-- Human -> Item Teleports (Đã cập nhật danh sách Item mới của KP)local HumanItemSection = HumanTab:CreateSection("Human Item Teleports")local HumanItems = {"Hazmat Suit", "Medkit", "Katana", "Machete", "Knife","Brick", "Nunchucks", "Sledge Hammer", "Crowbar", "Long Pipe", "Bat"}for _, itemName in ipairs(HumanItems) doHumanItemSection:AddButton({Name = itemName,Callback = function() TeleportToNamedItem(itemName) end})end-- Furry -> Basic Optionslocal FurryBasicSection = FurryTab:CreateSection("Furry/Goo Basic Options")FurryBasicSection:AddButton({Name = "Kill Yourself NOW!",Callback = function()local isAlive, _, hum = GetAliveCharacter(LocalPlayer)if isAlive and hum then hum.Health = 0 endend})-- Vòng lặp Noclip tối ưu hóa (Tránh gây giật lag FPS trên bản mới)RunService.Heartbeat:Connect(function()if Settings.DoorNoclip thenfor _, door in ipairs(Workspace:GetDescendants()) doif door:IsA("BasePart") and (door.Name:lower():find("door") or door.Name:lower():find("gate")) thendoor.CanCollide = falseendendendend)
