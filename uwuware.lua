...
-- small helper: treat Touch as primary click on mobile
local function isPrimaryClick(input)
...
local function createBind(option, parent)
	...
	local holding
	local loop
	-- support if option.key is an EnumItem or string
	local keyName = (typeof(option.key) == "EnumItem") and option.key.Name or tostring(option.key or "F")
	local text = string.match(keyName, "Mouse") and string.sub(keyName, 1, 5) .. string.sub(keyName, 12, 13) or keyName
	...
	main.InputEnded:connect(function(input)
		if isPrimaryClick(input) then
			-- begin binding state (user tapped the bind area)
			binding = true
			bindinput.Text = "..."
			tweenService:Create(round, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = Color3.fromRGB(255, 65, 65)}):Play()
		end
	...
	inputService.InputBegan:connect(function(input)
		...
		-- check for activation (non-binding)
		local matched = false
		pcall(function()
			if input.KeyCode and option.key and tostring(input.KeyCode.Name) == tostring(option.key) then matched = true end
		end)
		pcall(function()
			if input.UserInputType and option.key and tostring(input.UserInputType.Name) == tostring(option.key) then matched = true end
		end)
		-- allow Touch to match if option.key was set to "Touch"
		if input.UserInputType == Enum.UserInputType.Touch and tostring(option.key) == "Touch" then matched = true end

		if matched and not binding then
			if option.hold then
		...
		elseif binding then
			-- user is binding: capture either KeyCode or Touch/UserInputType
			local key
			-- prefer KeyCode if available and not blacklisted
			pcall(function()
				if input.KeyCode and not keyCheck(input.KeyCode, blacklistedKeys) then
					key = input.KeyCode
				end
			end)
			-- accept touch or mouse button inputs
			pcall(function()
				if (input.UserInputType == Enum.UserInputType.Touch) and not key then
					key = Enum.UserInputType.Touch
				end
			end)
			pcall(function()
	...
	inputService.InputEnded:connect(function(input)
		-- if a hold loop was created, stop it when the input ends
		local shouldMatch = false
		pcall(function()
			if input.KeyCode and tostring(input.KeyCode.Name) == tostring(option.key) then shouldMatch = true end
		end)
		pcall(function()
			if input.UserInputType and tostring(input.UserInputType.Name) == tostring(option.key) then shouldMatch = true end
		end)
		if input.UserInputType == Enum.UserInputType.Touch and tostring(option.key) == "Touch" then shouldMatch = true end
	...
	function option:SetKey(key)
		...
		-- If key is an EnumItem, use its name for storage/display; otherwise tostring
		self.key = (typeof(self.key) == "EnumItem") and self.key.Name or tostring(self.key)
...
local function createSlider(option, parent)
	...
	inputService.InputChanged:connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement and sliding then
			option:SetValue(option.min + ((input.Position.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X) * (option.max - option.min))
		elseif input.UserInputType == Enum.UserInputType.Touch and sliding then
			-- on touch, this will still provide Position when dragging
			option:SetValue(option.min + ((input.Position.X - slider.AbsolutePosition.X) / slider.AbsoluteSize.X) * (option.max - option.min))
		end
	end)
...
local function createColorPickerWindow(option)
	...
	inputService.InputChanged:connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseMovement and editinghue then
			...
			X = X <= 0 and 0 or X >= 0.995 and 0.995 or X
			option:updateVisuals(Color3.fromHSV(1 - X, sat, val))
		elseif Input.UserInputType == Enum.UserInputType.Touch and editinghue then
			X = (option.hue.AbsolutePosition.X + option.hue.AbsoluteSize.X) - option.hue.AbsolutePosition.X
			X = (Input.Position.X - option.hue.AbsolutePosition.X) / X
			X = X <= 0 and 0 or X >= 0.995 and 0.995 or X
			option:updateVisuals(Color3.fromHSV(1 - X, sat, val))
	...
	inputService.InputChanged:connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseMovement and editingsatval then
			...
			Y = Y <= 0 and 0 or Y >= 0.995 and 0.995 or Y
			option:updateVisuals(Color3.fromHSV(hue, X, 1 - Y))
		elseif Input.UserInputType == Enum.UserInputType.Touch and editingsatval then
			X = (option.satval.AbsolutePosition.X + option.satval.AbsoluteSize.X) - option.satval.AbsolutePosition.X
			Y = (option.satval.AbsolutePosition.Y + option.satval.AbsoluteSize.Y) - option.satval.AbsolutePosition.Y
			X = (Input.Position.X - option.satval.AbsolutePosition.X) / X
			Y = (Input.Position.Y - option.satval.AbsolutePosition.Y) / Y
...
local function getFnctions(parent)
	...
	function parent:AddBind(option)
		option = typeof(option) == "table" and option or {}
		option.text = tostring(option.text)
		-- keep option.key as either EnumItem or string; default to "F"
		if typeof(option.key) == "EnumItem" then
			-- keep as-is
		else
			option.key = (option.key and tostring(option.key)) or "F"
		end
		...
		option.position = #self.options
		option.flag = option.flag or option.text
		-- store flag as present key name/string
		library.flags[option.flag] = (typeof(option.key) == "EnumItem" and option.key.Name) or tostring(option.key)
		table.insert(self.options, option)
...
function library:Init()
	...
	self.cursor.Visible = not isTouch
...
function library:Close()
	...
	if self.cursor then
		self.cursor.Visible = self.open and not isTouch
	end
...
-- keep track of last touch/mouse position for cursor fallback (if needed)
local lastPointerPosition = Vector2.new(0,0)

inputService.InputChanged:connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseMovement and library.cursor and not isTouch then
		local mouse = inputService:GetMouseLocation() + Vector2.new(0, -36)
		library.cursor.Position = UDim2.new(0, mouse.X - 2, 0, mouse.Y - 2)
	elseif input.UserInputType == Enum.UserInputType.Touch and library.cursor and not isTouch then
		-- if for some reason cursor visible and touch occurs, update with touch position
		local pos = input.Position
		library.cursor.Position = UDim2.new(0, pos.X - 2, 0, pos.Y - 2)
	end
	-- save last pointer position for use elsewhere
